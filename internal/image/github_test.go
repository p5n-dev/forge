...
func TestGitHubReleasesSource_LatestVersion(t *testing.T) {
	srv := newGitHubServer(t, "schubergphilis", "forge", []fakeRelease{
		{tag: "v0.2.0"},
		{tag: "v0.1.0"},
	})

	src := image.NewGitHubReleasesSource("schubergphilis", "forge",
...
func TestGitHubReleasesSource_LatestVersion_NotFound(t *testing.T) {
	srv := newGitHubServer(t, "schubergphilis", "forge", nil)
...
func TestGitHubReleasesSource_DefaultRepo(t *testing.T) {
	...
	owner, repo := src.Repo()
	assert.Equal(t, "schubergphilis", owner)
